...
export default function ext({ translator, shouldHide, senseNavigation }) {
  ...
  if (shouldHide.isEnabled('SENSECLIENT_IM_1525_BUTTON')) {
    ...
    definition.items.settings.items.background = {
      grouped: true,
      type: 'items',
    ...
    definition.items.settings.items.border = {
      type: 'items',
      grouped: true,
    ...
    definition.items.settings.items.general.items.label = {
      component: 'string',
      ref: 'style.label',
      translation: 'Common.Label',
      expression: 'optional',
    }
    definition.items.settings.items.icon = {
      type: 'items',
      grouped: true,
      translation: 'properties.icon',
      items: {
