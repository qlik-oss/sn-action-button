...
export default {
  ...
  getStylesStylingPanel({ style = {}, disabled, theme, element, app, layout }) {
    ...
    // background
    const backgroundColor = getColor(bgColor, primaryColor, theme);
    styles += formatProperty('background-color', backgroundColor);
    if (bgImage.mode === 'media' && bgImage.mediaUrl.qStaticContentUrl) {
      ...
      if (bgUrl) {
        bgUrl = urlUtils.getImageUrl(bgUrl, app);
        styles += formatProperty('background-image', `url('${bgUrl}')`);
        styles += formatProperty('background-size', backgroundSize[btnStyleComp?.bgImage?.sizing === 'originalSize' ? 'auto' : btnStyleComp?.bgImage?.sizing || DEFAULTS.BACKGROUND_POSITION]);
        styles += formatProperty(
          'background-position',
