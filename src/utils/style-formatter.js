...
const formatColorProperty = (path, inputColor) => {
...
export default {
  getStyles(style, Theme) {
    ...
    styles += formatColorProperty('color', style.fontColor);
    styles += formatProperty('font-size', style.fontSize ? `${style.fontSize}px` : '12px');
    styles += formatColorProperty('background-color', style.backgroundColor);
